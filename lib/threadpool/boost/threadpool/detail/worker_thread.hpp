  ...
  */ 
  ...
  public:
	  ...
	  */
	  void run()
	  { 
		  mutex::scoped_lock lock(m_mutex); // wait for create_and_attach to finish 
		  std::cout << "worker run " << this << " use_count=" << this->shared_from_this().use_count() << std::endl;
		  std::cout << "m_thread " << m_thread.get() << " use_count=" << m_thread.use_count() << std::endl;
		  ...
		  std::cout << "end of worker run " << this << std::endl;
	  }
	  ...
	  */
	  static void create_and_attach(shared_ptr<pool_type> const & pool)
	  {
		  ...
		  std::cout << "worker create " << worker.get() << " use_count=" << worker.use_count() << std::endl;
// 		  worker_thread * worker = new worker_thread(pool);
// 		  std::cout << "worker create " << worker << std::endl;
		  if(worker)
		  {
			  mutex::scoped_lock lock(worker->m_mutex);
			  shared_ptr<boost::thread> thread(new boost::thread(bind(&worker_thread::run, worker)));
			  worker->m_thread = thread;
